...
# рассчитать урожайность пшеницы в 2013 году, 
...
omsk_around = meteo_nearby_stations(lat_lon_df = omsk, station_data = station_data, limit = 20, var = c("PRCP", "TAVG"), year_min = 2010, year_max = 2012)
# omsk_around это список единственным элементом которого является таблица, содержащая идентификаторы 
#метеостанций отсортированных по их 
# удалленности от Омска, очевидно что первым элементом таблицы будет идентификатор метеостанции Омска,
...
#2)чтобы получить таблицу всех метеостанций вокруг Омска нужно выбрать целиком первый объект из списка
omsk_table = omsk_around[[1]]
summary(omsk_table)
#в таблице оказалось20 объектов, ранжированных по расстоянию от Омска
...
omsk_stations$id
# в таблице оказалось 9 станций в нужном нам километраже
...
##нам нужны среднесуточные температуры (tavg) выше 5 градусов за  2010 - 12 гг.
...
#видим, что дата записана в формате "1963-01-02"
  ...
  all_omsk_meteodata [,"year"]= year(all_omsk_meteodata$date)
    ...
    years_omsk_meteodata = filter (all_omsk_meteodata, year > 2009 & year < 2013 )   
    #проверим результат
...
# максимальная суммарная температура за месяц 755,6, то есть 755,6/30=25,2
